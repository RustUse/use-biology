...
RustUse/use-biology keeps root workspace metadata at `publish = false` by default, while the current focused crate manifests and facade manifest opt in with `publish = true`. The release task is to verify that only the intended crates remain publishable.

Because `use-organism` depends on `use-taxonomy` and `use-species`, and the facade depends on every focused crate, the first publish wave is dependency ordered.
...
- keeps `crates/use-biology/Cargo.toml` at `publish = true`
...
- `.github/workflows/facade-publish-readiness.yml` is a manual post-publication check that dry-runs `use-biology` after the focused crates are live on crates.io.
...
- The root `CHANGELOG.md` remains the shared changelog and is updated through the `use-biology` package entry, including focused-crate commits.
...
- Configure crates.io Trusted Publishing for each published crate with repository owner `RustUse`, repository name `use-biology`, and workflow filename `release-plz-release.yml`.
...
5. Run the facade publish readiness workflow or `cargo publish --dry-run --allow-dirty -p use-biology`.
6. Publish `use-biology`.
...
9. Confirm focused crates plus `use-biology` are the only intentionally publishable crates.
