...
Use this checklist when adding a new focused crate or expanding the `use-biology` facade.
...
keywords = ["biology", "vocabulary", "rustuse"]
...
- Keep the crate out of bioinformatics, medical, taxonomy database, and simulation domains unless a future set explicitly owns that scope.
...
If a new focused crate should be available through `use-biology`, also update:

- root `Cargo.toml` workspace members, workspace dependencies, and first-publish metadata
- `crates/use-biology/Cargo.toml` dependencies
- `crates/use-biology/src/lib.rs` module aliases and prelude
- `crates/use-biology/README.md`
- root `README.md`
...
- Keep examples descriptive. They should not analyze, infer, simulate, diagnose, fetch, align, annotate, or resolve biological data.
