...
RustUse/use-biology is intentionally small. Contributions should favor durable vocabulary, clear naming, strong documentation, and minimal surface area over broad feature count.
...
4. Preserve the primitive-vocabulary boundary: no bioinformatics framework, genome analysis, diagnosis, taxonomy database, ecology simulator, lab information system, disease model, or biological simulation engine.
...
- Keep docs aligned with the current biology scope and non-goals.
...
- Publish focused crates before the `use-biology` facade. `use-organism` depends on `use-taxonomy` and `use-species`, so publish those first.
...
`release-plz` drives release PRs and changelog generation for publishable crates. Prefer commit subjects that match `type: summary` or `type(scope)!: summary`, such as `feat: add life-stage primitive`, `fix: reject empty tissue names`, or `docs: clarify bioinformatics boundary`.
