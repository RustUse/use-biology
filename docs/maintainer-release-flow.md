...
- The shared root `CHANGELOG.md` is generated through the `use-biology` package entry and includes focused-crate commits.
...
- Configure crates.io Trusted Publishing for every published crate with repository owner `RustUse`, repository name `use-biology`, and workflow filename `release-plz-release.yml`.
...
5. Run `cargo publish --dry-run -p use-biology` or the manual `Facade Publish Readiness` workflow.
