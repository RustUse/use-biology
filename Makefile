...
help:
	@printf "%s\n" \
		...
		"deny                           Run cargo-deny" \
		"sbom                           Generate a CycloneDX SBOM for use-biology" \
		"publish-dry-run-focused        List package contents and dry-run publish focused crates" \
		"publish-dry-run-facade         Dry-run publish use-biology after crates.io propagation" \
...
sbom:
	cargo cyclonedx --manifest-path crates/use-biology/Cargo.toml --all-features --format json --spec-version 1.5 --override-filename sbom.cyclonedx
...
publish-dry-run-facade:
	cargo publish --dry-run --allow-dirty -p use-biology
